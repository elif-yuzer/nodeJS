...
4.Kiler Sorumlusu (Model): Veritabanına (PostgreSQL) iner, SQL sorgularını çalıştırır, verileri alır ve Aşçıya teslim eder
...
7.Şimdi mutfağımızda malzemeleri getiren bir Kiler Sorumlusu (Model) ve yemeği hazırlamaya hazır bir Aşçı (Controller) var. Geriye müşterinin siparişini kapıdan alıp doğru aşçıya teslim edecek olan Garsonu (Route) görevlendirmek kaldı